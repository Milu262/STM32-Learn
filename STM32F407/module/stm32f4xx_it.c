...
//   {
//     test_data.flag = 1; // 接收完成标志 DMA_Cmd(DEBUG_USART_RX_DMA_STREAM,
//     DISABLE);                                       //
//     暂时关闭接收串口的DMA，数据待处理 usart1_rx_len = USART_MAX_LEN -
//     DMA_GetCurrDataCounter(DEBUG_USART_RX_DMA_STREAM); //
//     计算接收到的数据长度 DMA_ClearFlag(DEBUG_USART_RX_DMA_STREAM,
...
//     // else
//     // {
//     //   memcpy(test_data.data, DMA_USART1_RX_BUF, usart1_rx_len); //
//     将接收到的数据复制到test_data.data
//     //   test_data.data[usart1_rx_len] = '\0';
//     // }

//     DMA_SetCurrDataCounter(DEBUG_USART_RX_DMA_STREAM, USART_MAX_LEN); //
//     重新设置DMA的缓冲区大小 DMA_Cmd(DEBUG_USART_RX_DMA_STREAM, ENABLE); //
//     重新开启接收串口的DMA USART_ReceiveData(BSP_USART); // 清除接收中断标志
//   }

//   if (USART_GetITStatus(BSP_USART, USART_IT_TC) != RESET) // 串口发送完成
//   {

//     USART_ClearITPendingBit(BSP_USART, USART_IT_TC); //
//     清除串口发送完成标志位 USART_ITConfig(USART1, USART_IT_TC, DISABLE); //
//     关闭串口发送完成中断
...
void USART2_IRQHandler(void) {
  if (USART_GetITStatus(BSP_USART, USART_IT_IDLE) != RESET) // 触发空闲中断
  {
    ...
    DMA_SetCurrDataCounter(DEBUG_USART_RX_DMA_STREAM,
                           USART_MAX_LEN); // 重新设置DMA的缓冲区大小
...
// {
//   if (DMA_GetITStatus(DEBUG_USART_TX_DMA_STREAM, DMA_IT_TCIF7) != RESET) //
//   等待DMA发送完成中断
//   {
//     DMA_ClearITPendingBit(DEBUG_USART_TX_DMA_STREAM, DMA_IT_TCIF7); //
//     清除DMA发送完成中断标志 DMA_Cmd(DEBUG_USART_TX_DMA_STREAM, DISABLE); //
//     关闭DMA发送 USART_ITConfig(USART1, USART_IT_TC, ENABLE); //
//     使能串口发送完成中断
//   }
// }
...
void DMA1_Stream6_IRQHandler(void) {
  if (DMA_GetITStatus(DEBUG_USART_TX_DMA_STREAM, DMA_IT_TCIF6) !=
      RESET) // 等待DMA发送完成中断
  {
    DMA_ClearITPendingBit(DEBUG_USART_TX_DMA_STREAM,
                          DMA_IT_TCIF6); // 清除DMA发送完成中断标志
...
// {
//   if (DMA_GetITStatus(DCMI_DMA_STREAM, DMA_IT_TCIF1) != RESET) //
//   等待DMA传输完成中断,表示摄像头的一行数据已经传输完成
//   {
